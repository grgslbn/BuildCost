...
// idle → upload form
// processing → animated show + polling
// slow_lead → backend >120s with no result, ask for email (Scenario C)
// results → teaser + lead capture
// error_lead → backend failed, ask for expert review (Scenario D)
type Phase = "idle" | "processing" | "slow_lead" | "results" | "error_lead";
...
export default function RealUploadDemo() {
  ...
  const [pillEmailSubmitted, setPillEmailSubmitted] = useState(false);
  const [processShowEl, setProcessShowEl] = useState<HTMLElement | null>(null);
  ...
  // Polling (also keeps running once we've degraded to slow_lead so that the
  // "surprise — results are here!" reveal works).
  useEffect(() => {
    const isPolling = phase === "processing" || phase === "slow_lead";
    if (!isPolling || !estimationId) return;
    let cancelled = false;
  ...
  // Scenario C: if the backend is taking too long (> SLOW_THRESHOLD_MS) and
  // we still don't have results, degrade to a "leave email" form. Polling
  // continues — when results finally arrive, we surprise-reveal the teaser.
  useEffect(() => {
    if (phase !== "processing" || !processingStartedAt) return;
    const elapsed = Date.now() - processingStartedAt;
    const delay = Math.max(0, SLOW_THRESHOLD_MS - elapsed);
    const id = setTimeout(() => {
      if (liveData?.status !== "complete" && liveData?.status !== "error") {
        setPhase("slow_lead");
      }
    }, delay);
    return () => clearTimeout(id);
  }, [phase, processingStartedAt, liveData?.status]);

  // When in slow_lead and results arrive, jump straight to results
  useEffect(() => {
    if (phase === "slow_lead" && liveData?.status === "complete") {
      setPhase("results");
    }
  ...
  if (phase === "processing") {
    return (
      <>
        <div ref={setProcessShowEl} className="real-upload-zone real-upload-processing">
          <ProcessShow
            liveData={liveData}
        ...
        <ProcessingPill
          ...
          status={liveData?.status ?? "uploading"}
          processShowEl={processShowEl}
          onSubmitted={() => { setPillEmailSubmitted(true); setUnlocked(true); }}
        />
      </>
    );
  }
  ...
  if (phase === "slow_lead") {
    return (
      <div className="real-upload-zone real-upload-results">
        <div className="rud-soft-landing">
          <h3>Your plan requires a deeper analysis</h3>
          <p>
            {pillEmailSubmitted
              ? "We've got your email — we'll send your complete rebuild cost estimate within the hour."
              : "Complex plans take a bit longer. Leave your email and we'll send your complete rebuild cost estimate within the hour."}
          </p>
        </div>
        {!pillEmailSubmitted && (
          <LeadCapture
            estimationId={estimationId}
            mode="pending"
            onUnlock={() => { /* keep waiting; polling will reveal results */ }}
          />
        )}
        <div className="reset-row">
          <button className="btn btn-secondary" onClick={reset}>↺ Try a different plan</button>
        </div>
      </div>
    );
  }
  ...
  if (phase === "error_lead") {
    return (
      <div className="real-upload-zone real-upload-results">
        <div className="rud-soft-landing">
          <h3>We couldn't fully analyze this plan automatically</h3>
          <p>
            {pillEmailSubmitted
              ? "We've got your email — our team will review your plan and send you a detailed estimate within one business day."
              : "This sometimes happens with façade drawings, renovation plans, or very large documents. Leave your email and our team will review your plan and send you a detailed estimate."}
          </p>
          {errorMsg && <div className="rud-soft-error">Technical detail: {errorMsg}</div>}
        </div>
        {!pillEmailSubmitted && (
          <LeadCapture
            estimationId={estimationId}
            mode="expert_review"
            onUnlock={() => { /* expert will follow up via email */ }}
          />
        )}
        <div className="reset-row">
          <button className="btn btn-secondary" onClick={reset}>↺ Try a different plan</button>
