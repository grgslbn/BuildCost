...
const AKEY = env.ANTHROPIC_API_KEY; const FILE = process.argv[2]; const sleep = ms => new Promise(r=>setTimeout(r,ms));
const doc = mupdf.Document.openDocument(readFileSync(FILE), "application/pdf");
async function overview(p){const pix=doc.loadPage(p).toPixmap(mupdf.Matrix.scale(150/72,150/72),mupdf.ColorSpace.DeviceRGB,false,true);let png=Buffer.from(pix.asPNG());png=await sharp(png).resize({width:1568,height:1568,fit:"inside",withoutEnlargement:true}).png().toBuffer();return png.toString("base64");}
...
{"sheet_type":"floor_plan|section|elevation|site|title|detail|area_table|other","floor_label":"...","scanned_or_cad":"scanned|cad",
 "has_printed_unit_or_room_areas":<bool>,"area_examples":["leefruimte 30 m²", ...up to 5],"has_dimensions":<bool>,"legible":"high|med|low","note":"short"}`;
...
for(let p=0;p<doc.countPages();p++){try{const r=await vision(await overview(p));console.log(`p${p}: ${r.sheet_type} [${r.floor_label||''}] ${r.scanned_or_cad} | areas:${r.has_printed_unit_or_room_areas} dims:${r.has_dimensions} ${r.legible}` + (r.area_examples?.length?` | ${r.area_examples.slice(0,4).join("; ")}`:""));}catch(e){console.log(`p${p} FOUT ${String(e.message).slice(0,80)}`);}await sleep(800);}
