...
- **SQM ROUTER** (2026-06-01, wired in `run-estimation.ts` → also live in the Railway worker via shared pipeline): input is heterogeneous (PDF or **JPEG**, with/without an area table, with/without printed m² labels), so the pipeline picks the **best available signal per dossier** and is honest about confidence. Tiers, highest-reliability first:
  ...
  - **Tier 3 — `chain_reader` (agentic, 2026-08-09) → fallback `bare_plan`.** Behind `CHAIN_READER_ENABLED=true` (**Railway worker only** — 5–20 min/dossier, never Vercel): `src/lib/sqm/chain-reader.ts` runs an agentic tool-loop (view_page/crop/report, claude-opus-5, prompt caching, JPEG q82) that reads DIMENSION CHAINS the way a human expert does — closes segment sums on printed end measures, counts floors from sections, applies the convention handbook (terrace/roof/basement/carport/estate/function rules distilled from expert berekeningen). Deterministic verification (`verifyChainReport`) + confidence + lower-bound flags gate AUTO (conf 0.8) vs manual panel (0.45). **Dev-set (12 dims-only dossiers): median |Δcost| 4.1%, all AUTO within ±10% cost-weighted** (one +11% = GT-convention dispute, cat1 −0.3%); hard cases (kasteeldomein, toren) correctly gated. Research harness: `scripts/chain-reader.mjs` + `scripts/verify-chain.mjs` (blind re-measure of 2 largest cat1 parts = strongest gate, not yet in pipeline), `scripts/backtest-chain.mjs`, `scripts/aggregate-chain.mjs`, GT via `scripts/harvest-gt.mjs` → `gt-auto.json`. When the flag is off: the v9 measurement + **confidence-gating** (`sqm-confidence.ts`), ~38%, LOW confidence, flagged for manual review.
...
- **⚠ The unit-only factor is NOT a constant 1.35 — it spans ~1.12…1.35 with the circulation share.** Blind test "Hendrik I Lei" (corner block, 2 compact cores serving 5-6 apts/level): printed unit nets 401/level ×**1.12** = 449 = expert 448 exactly — ×1.3 overshot cat1 +18% (total +27%). Schaarbeek 23-499974 (10 small units, high circulation share) genuinely needed ×1.35. Expert there also priced low (€1,848/m², F≈0.86) — F band for standard apartments is wider than 0.95–1.0. **IMPLEMENTED (2026-06-10)**: in the unit_net case the model now ESTIMATES circulation explicitly from the drawing (`circulation_m2` + `n_cores`: cores ≈12–25 m²/core/floor + corridors, geometry-derived, no percentage guessing) and the code computes `cat1 = (unit nets + circulation) × 1.12`; sanity-guard (estimate must be >0 and <50% of nets) else fall back to ×1.35 (validated on 23-499974). Hendrik I Lei replay: (401+~30)×1.12 ≈ 483 vs expert 448 (+8%, was +18%). `circulationM2` exposed for telemetry. **VALIDATED BLIND (Eupener Strasse, 16 Wohnungen): room-sums + explicit cores → cat1 −4%, unit count 16/16 exact.** Remaining gaps there: sous-sol footprint −24% (basement deserves the same dimension-chain rigor) and price level (see m³ note below).
- **Third CED calculation convention: VOLUME-based (French template)** — "Calcul/Sous-sols/Bâtiment, Surf/contenu in m³ at €750/m³ + €25,000 per unit 'concentration d'équipements'" (Eupener Strasse: 5,528 m³ + 16×25k; sous-sol by m² @ €1,048). Effective above-ground ≈ €2,300–2,500/m² — ~15% above the de-biased standard €2,040. Route-A table parsers must recognise m³ rows (Hauteur ≠ 0 → volume, divide by height for floor-equivalent m²).
...
- **CED pricing conventions discovered via 23-483997** (expert berekening, Vallei Houtkauter): (1) **"Inpandige terrassen" (recessed/inset terraces, inside the building volume) are priced at the FULL woon rate (€2,000/m²), NOT the terras rate** — our classifier putting every "terras" in cat3 undercounts ~€780k on that dossier; rule: inpandig/inset terras → cat1-price, free-hanging balkon/dakterras → cat3 (€502). (2) **Groendak = separate cheap line (€200/m²)** — must NOT land in cat3 (€700 overprices); classifier already routes groendak→cat3, better: exclude or separate. (3) Parkeerkelder €1,155 ≈ our decoupled cat2 €1,200 ✓; woon €2,008 = lean F≈0.95 ✓ (de-biased intercept confirmed again).
- **Direct model reading of dims-only plans (Fable, 2026-06)**: reading dimension chains + closing them on totals + counting floors from sections — the human approach — validated blind on 2 dossiers: 26-552710 cat1 −0.5% (3×260 footprint), 23-483997 cat1 **−4.2%** + cat2 +5.2% (calibrated on 2.5m parking bays when chains unreadable). The API-called pipeline models still fail this input type; the interactive model does not. Candidate: upgrade bare_plan tier to this approach/model.
- **Scanned/image PDFs** (e.g. 550471): `pdftotext` finds nothing → classify pages + extract via VISION only.
- **Partial excerpts are a LOWER BOUND — flag missing levels hard** (blind test 24-522268): the upload had 1 plan sheet, no kelder plans/gevels/sneden → **765 m² underground (40% of all m², €1.8M) was invisible** → estimate −75%. Rule: if no basement plan AND no section/elevation is present, say explicitly "ondergrondse niveaus onzichtbaar — schatting is ondergrens" and cap confidence.
- **Ultra-luxury segment sits structurally above the model** (24-522268, concrete rates): woning **€5,300/m²**, poolhouse €4,000, finished kelder €2,500, schuur €3,200 — far beyond cat1_max €2,900 (the intentionally kept cap). Even with perfect m² the model is ~−45% on €/m² for this tail. Indicators to FLAG instead of price: autolift, binnenzwembad, personenlift, poolhouse in a private villa. Inrichting there (autolift 110k + zwembad 90k + sauna 25k + personenlift 40.5k) again outside the m² model.
...
**DE-BIASED 2026-06-01 → v102 (intercept 1.0274).** The +19% overshoot was confirmed live: the model's F-median across 183 estimations was **1.295** (apartments 1.339) vs the CED-correct 1.04. So the intercept was lowered uniformly by **0.255** (1.2824→1.0274), centering the F-median on CED's 1.04 (€2150). Effect: every F drops 0.255. **Confirmed live on 2 dossiers vs expert €/m²:** 23-499974 cat1 €/m² −6% (was +16%); 23-499978 cat1 €/m² **+0.6%** (€1969 vs expert €1957) — both within ±6%, the +19% bias gone. Totals: 23-499974 −13%→capture-bound; 23-499978 **−7%** total (cat3 +2%, cat2 +16%, cat1 m² −11% = missed gemene delen, + €221k lift/fietsenberging structurally outside the m² model). Calibrated/applied via `scripts/calibrate-intercept.mjs` + `f-current.mjs` (service-role; the MCP is read-denied). v101 weights unchanged; rollback = reactivate v101. (`evaluation_results` predicted/expert_f is STALE — old runs — don't calibrate on it.)
